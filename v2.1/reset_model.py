...
# ─────────────────────────────────────────────────────────────────────────────
# v2.1 reset_model.py
# Initialises weights for the PyTorch-accelerated model.
# Architecture:
#   Input  : 28x28 image
...
H2 = torch.randn(H1_NEURONS, 10)     * (2.0 / H1_NEURONS) ** 0.5
...
config = {"NUM_KERNELS": NUM_KERNELS, "H1_NEURONS": H1_NEURONS}
torch.save(config, os.path.join(target_dir, "config.pt"))

print(f"Weights reset  →  K:{list(K.shape)}  H1:{list(H1.shape)}  H2:{list(H2.shape)}")
