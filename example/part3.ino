...
bool readImu6_raw(int16_t *ax, int16_t *ay, int16_t *az,
  ...
  I2C_start();
  if(!I2C_write_byte((MPU_ADDR << 1) | 0)) return false; 
  if(!I2C_write_byte(ACCEL_XOUT_H)) return false; 
  

  I2C_repeated_start(); 
  if(!I2C_write_byte((MPU_ADDR << 1) | 1)) return false; 
...
void recordOneSample() {
  ...
  for (int i = 0; i < NUM_SAMPLES; i++) {
    ...
    /* Your implementation */
    int16_t ax, ay, az, gx, gy, gz; // declare
    // ****************************************************************************
