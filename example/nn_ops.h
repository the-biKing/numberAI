...
inline void preprocess_input(
  ...
  for (int i = 0; i < NN_INPUT_LEN; i++) {
    for (int c = 0; c < NN_INPUT_CH; c++) {
      ...
      float physical;
      //physics tranfer
      if (c < 3) physical = (float)raw[i][c] / 16384.0f; //acc
      else       physical = (float)raw[i][c] / 131.0f;   //g

      float standardized = (physical - NN_MEAN[c]) / NN_STD[c];

      //quantization
      int32_t q = (int32_t)roundf(standardized / SCALE_INPUT);
      if (q > 127) q = 127;
      if (q < -128) q = -128;
...
inline int8_t requantize_int8(int32_t acc, int32_t mult, uint8_t shift) {
  // Your implementation
  //int64_t prevent overflow
  int64_t scaled = (int64_t)acc * mult;

  int32_t res = (int32_t)(scaled >> shift);
  //clamp
  if (res > 127) res = 127;
  if (res < -128) res = -128;
