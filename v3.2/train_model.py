...
print(f"嗷嗚！發現了 {num_train} 隻訓練獵物和 {len(x_test)} 隻測試獵物！")
...
def save_model(model):
    ...
    np.savetxt(os.path.join(hidden_layer_dir, "residual.txt"), model.W_skip.data.cpu().numpy(), fmt='%f')
    print("🐾 肌肉記憶已深深烙印在洞穴石壁上！(Model saved to disk successfully)")
...
print("🐾 進入狂暴狩獵模式 (PyTorch training)... 按下 Ctrl+C 可隨時中斷！")
...
# 🐾 把 19 條神經通通打包進一個 list
H1_params = [
    model.H1_1, model.H1_2, model.H1_3, model.H1_4, model.H1_5, model.H1_6, model.H1_7,
    model.H1_8, model.H1_9, model.H1_10, model.H1_11,
    model.H1_12, model.H1_13, model.H1_14, model.H1_15,
    model.H1_16, model.H1_17, model.H1_18, model.H1_19
]
...
try:
    for epoch in range(epochs):
        ...
        print(f"第 {epoch+1:03d}/{epochs} 輪狩獵 - Train Loss: {train_loss:.4f}, Train Acc: {train_acc*100:.2f}% | Test Acc: {test_acc*100:.2f}% | 消耗能量 (Energy): {energy:.4f}")
...
print(f"呼哈～ 狩獵結束！總共耗費了 {total_time:.2f} 秒的體力。")
...
plt.ioff()
print("看著這張美麗的學習曲線圖吧！關閉視窗後就會結束程式喔。")
