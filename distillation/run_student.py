...
def load_local_weights(model, hidden_layer_dir):
    try:
...
if __name__ == "__main__":
    ...
    if not load_local_weights(model, hidden_layer_dir):
        print("Error: Distilled weights not found in hiddenLayer/. Run reset_student.py first.")
        sys.exit(1)
        
    print("Loading weights...")
    total_params = sum(p.numel() for p in model.parameters())
    print(f"Student Model Size: {total_params:,} parameters")
    ...
    if len(sys.argv) > 1:
        ...
        print(f"[EMNIST Test] Distilled Student Accuracy: {correct}/{num_samples} ({accuracy:.2f}%)")
        print(f"Total Time: {t1 - t0:.4f}s ({(t1 - t0)*1000/num_samples:.2f} ms/image)")
