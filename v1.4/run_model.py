...
def run_inference(image_path, H1, H2, quiet=False):
    ...
    I_16x16 = I.reshape(1, 16, 16)
    pool1 = I_16x16.reshape(1, 8, 2, 8, 2).max(axis=(2, 4))
    
    # 3x3 Convolution with fixed Laplacian kernel
    ...
    conv_out = np.zeros((1, 6, 6), dtype=np.float32)
    for i in range(3):
        for j in range(3):
            conv_out += pool1[:, i:i+6, j:j+6] * laplacian_kernel[i, j]
            
    pool_conv_flat = conv_out.flatten()
